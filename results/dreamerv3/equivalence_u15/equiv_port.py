...
import os
raw = {"preset": "atari100k", "batch_size": 2, "batch_length": 8, "compute_dtype": "float32", "device": DEVICE}
if os.environ.get("WARMUP") is not None:
    raw["agent"] = {"opt": {"warmup": int(os.environ["WARMUP"])}}
cfg = C.config_from_dict(raw)
print("warmup", cfg.agent.opt.warmup)
torch.manual_seed(0)
agent = Agent((64, 64, 3), 18, cfg)
...
for key, tensor in sd.items():
    ...
    loaded[key] = torch.as_tensor(src).to(tensor.dtype)
    used.add(name)
